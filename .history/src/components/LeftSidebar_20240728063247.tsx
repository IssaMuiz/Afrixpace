...
const LeftSidebar = (props: Sidenavtype) => {
  ...
  return (
    <div
      className={`w-72 desktop:flex p-10 pl-12 backdrop-blur-3xl flex-col border-r bg-[#313131]  bg-opacity-30 mt-16  fixed z-0 h-full left-0 overflow-scroll top-0 scrollbar ${
                ...
                <div className="border-l ml-8">
                  {category.dropdownLink.map((dropDownlink) => (
                    <Link
                      ...
                      className=" flex font-bold text-white flex-col gap-y-2 pl-4 p-2 transition-transform duration-300 mb-5 hover:bg-[#313131] rounded-lg cursor-pointer"
                    >
                      {dropDownlink.name}
                    </Link>
                  ))}
