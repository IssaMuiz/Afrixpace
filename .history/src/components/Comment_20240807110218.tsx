...
const Comment = (props: PostType) => {
  const commentRef: any = collection(storage, "posts", props.postId, "comment");
  ...
  const handlesetComment = (e: any) => {
    const set = setNewcomment(e.target.value);
  };
      ...
      const unsubscribe = onSnapshot(q, (snapshot) => {
        const filteredData = snapshot.docs.map((doc) => {
          ...
          if (docData && typeof docData === "object") {
            ...
            console.error("Document data is not an object:", docData);
          }
        });
